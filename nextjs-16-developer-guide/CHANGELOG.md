...
- Full production manuscript content for **Part I — Modern Next.js**:
  - `01-what-nextjs-16-is.md`: Next.js 16.3.4 & React 19 architecture, RSC binary payload streaming, Turbopack default engine, explicit caching, and Pages vs App router analysis.
  - `02-creating-a-nextjs-16-project.md`: Scaffolding, `tsconfig.json` bundler resolution, `next.config.ts`, environment schema validation, and CI checks.
  - `03-understanding-the-app-router.md`: Special file conventions, nested layout state preservation, async dynamic `params`/`searchParams` contract, and error boundaries.
- Outline template files for Chapters 4–41 and Appendices A–F in `content/chapters/`.
- Compilable Next.js 16.3.4 App Router reference application ("Acme App") in `examples/acme-app/` featuring environment configuration schemas (`lib/env.ts`) and `next.config.ts`.
